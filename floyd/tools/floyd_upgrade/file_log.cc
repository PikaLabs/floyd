...
void ReplayEntry(rocksdb::DBNemo* db, Entry& entry) {
  if (entry.type() == Entry_Type::Entry_Type_DATA) {
    command::Command cmd;
    rocksdb::Status ret;
    if (cmd.ParseFromString(entry.cmd())) {
      switch (cmd.type()) {
        case command::Command_Type::Command_Type_Write:
          ret = db->Put(rocksdb::WriteOptions(), cmd.kv().key(), cmd.kv().value());
          if (ret.ok()) {
            printf ("[      ] Write %s, key(%s) value(%s)\n", ret.ToString().c_str(), cmd.kv().key().c_str(), cmd.kv().value().c_str());
          } else {
            printf ("[Failed] Write %s, key(%s) value(%s)\n", ret.ToString().c_str(), cmd.kv().key().c_str(), cmd.kv().value().c_str());
          }
          break;
        case command::Command_Type::Command_Type_Delete:
          ret = db->Delete(rocksdb::WriteOptions(), cmd.kv().key());
          if (ret.ok()) {
            printf ("[      ] Delete %s, key(%s)\n", ret.ToString().c_str(), cmd.kv().key().c_str());
          } else {
            printf ("[Failed] Delete %s, key(%s)\n", ret.ToString().c_str(), cmd.kv().key().c_str());
          }
          break;
        default:
          printf ("[Unknow] Other cmd type %d\n", cmd.type());
      }
    }
  }
}
