...
bool StateMachine::WaitForReadResponse(uint64_t log_index, std::string& key,
  ...
  Status ret = Floyd::db->Get(key, value);
  return true;
}
...
bool StateMachine::WaitForReadAllResponse(uint64_t log_index, KVMap& kvmap) {
  ...
  Status ret = Floyd::db->GetAll(kvmap);
  return true;
}

Status StateMachine::WaitForTryLockResponse(uint64_t log_index) {
  ...
  return Status::Corruption("fetch result failed");
}

Status StateMachine::WaitForUnLockResponse(uint64_t log_index) {
...
Status StateMachine::WaitForDeleteUserResponse(uint64_t log_index) {
...
bool StateMachine::ApplyThread::Apply(StateMachine::Entry& entry) {
  raft::Entry log_entry = entry.log_entry;
  if (log_entry.type() == raft::Entry::NOOP) return true;
  ...
  switch (cmd.type()) {
    ...
    case command::Command::Delete: {
      command::Command_Kv kv = cmd.kv();
      std::string key = kv.key();
      Status ret = Floyd::db->Delete(key);
      return ret.ok();
    }
    ...
    case command::Command::TryLock: {
      ...
      int lock_is_available = Floyd::db->LockIsAvailable(user, key);
      ...
      if (lock_is_available == 1) {
        ...
        Status ret = Floyd::db->LockKey(user, key);
        if (!ret.ok()) {
          result.status_ = ret;
        }
      } else if (lock_is_available == 0) {
        result.status_ = Status::Timeout("already locked");
      }
    ...
    case command::Command::UnLock: {
      ...
      if (lock_is_available == 0) {
        Status ret = Floyd::db->UnLockKey(user, key);
        if (!ret.ok()) {
          result.status_ = ret;
        }
      ...
      // std::string value;
      // Status ret = Floyd::db->Get(lock_key, value);
    ...
    case command::Command::DeleteUser: {
      ...
      Status ret = Floyd::db->DeleteUserLocks(user);
      if (!ret.ok()) {
        result.status_ = ret;
      }
...
void* StateMachine::ApplyThread::ThreadMain() {
  while (!sm_->exiting_) {
    ...
    LOG_DEBUG(
        "ApplyThread::ThreadMain: GetNextCommitIndex last_apply_index=%ld entry.index=%ld",
